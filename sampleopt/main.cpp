...
void queries_from_parser (void)
{
  ...
  if (yyparse ())
    {std::cout << "parsing failed" << std::endl; return;}
  else
     ; // std::cout << "parsing succeeded" << std::endl;

  Bparser_state()->query.typechk ();
  if (Bparser_state()->error ())
    {std::cout << Bparser_state()->error_str () << std::endl; return;}
  else
    ; // std::cout << "typechk succeeded" << std::endl;
...
void generate_and_execute (int num_rels, int num_preds, int unparse)
{
  ...
  if (unparse)
    {
      std::cout << "The query was: " << std::endl;
      rqg.unparse (std::cout);
    }
...
int my_getopt (int argc, char *argv[], char *)
{
  ...
  if (option_string[1] != '\0')
    {
      std::cout << "unknown option " << option_string << std::endl;
      return '?';
    }
